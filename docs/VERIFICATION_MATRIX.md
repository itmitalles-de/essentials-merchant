...
Status recorded on 2026-08-20 on branch `pilot/mantle-amazon-analysis-live`. Every local login,
...
| --- | --- | --- | --- | --- | --- | --- |
| Rust Core/domain/PDF | fmt; Clippy `-D warnings`; 88 tests | PostgreSQL 16 migrations 1–18; SQLx prepare/check | fixture data | no | no | Deployment sizing and production data remain untested |
...
| Upgrade rehearsal | Migration assertions | Synthetic v10 invoice/report data migrates losslessly through migration 18 | synthetic | n/a | no | No production or Vendure major-version upgrade performed |
...
- Backend: `cargo fmt --all -- --check`; SQLx-offline Clippy for all workspace targets with warnings
  denied; migrations 1–18 and `cargo sqlx prepare --workspace --check` against disposable
  PostgreSQL 16; 19 DB + 13 domain + 8 PDF + 48 server tests = **88 passed**.
- Frontend: clean npm install, production build and lint pass. The one Playwright Chromium pilot
  ...
  export, disabled OpenAI gate, synthetic weekly strategy/handover rendering, fixed KPI charts,
  AI-first route, eight
  HTTP 409 boundary probes and zero serious/critical axe violations.
