...
| --- | --- | --- | --- | --- | --- | --- |
| Rust Core/domain/PDF | fmt; Clippy `-D warnings`; 87 tests | PostgreSQL 16 migrations 1–17; SQLx prepare/check | fixture data | no | no | Deployment sizing and production data remain untested |
...
| Optional AI strategy | Closed aggregate DTO/hash, strict schema, evidence-reference and disabled-gate tests pass | Browser covers disabled gate, explicit confirmation and structured synthetic output separated from facts | local fake Responses API only | no | no | Separately billed API key, provider data controls, quality and cost remain external |
...
| Upgrade rehearsal | Migration assertions | Synthetic v10 invoice/report data migrates losslessly through migration 17 | synthetic | n/a | no | No production or Vendure major-version upgrade performed |
...
- Backend: `cargo fmt --all -- --check`; SQLx-offline Clippy for all workspace targets with warnings
  denied; migrations 1–17 and `cargo sqlx prepare --workspace --check` against disposable
  PostgreSQL 16; 19 DB + 13 domain + 8 PDF + 47 server tests = **87 passed**.
- Frontend: clean npm install, production build and lint pass. The one Playwright Chromium pilot
  ...
  export, disabled OpenAI gate, confirmed synthetic strategy rendering, AI-first route, eight
...
- Upgrade: migration-10 synthetic invoice/report data reaches migration 17 losslessly.
