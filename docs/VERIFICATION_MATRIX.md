...
| --- | --- | --- | --- | --- | --- | --- |
| Rust Core/domain/PDF | fmt; Clippy `-D warnings`; 95 tests | PostgreSQL 16 migrations 1–19; SQLx prepare/check | fixture data | no | no | Deployment sizing and production data remain untested |
...
| React Admin / accessibility | TypeScript/Vite build and lint pass | Two Chromium flows cover no-login, write-only credentials, report/import/polling/snapshot/analysis/export and axe with no serious/critical finding | fixture SP-API | no | no | Three retained non-failing Fast Refresh lint warnings; no real-provider UX claim |
...
| Optional AI strategy | Closed aggregate-history DTO/hash, previous-handover minimization, strict schema, evidence-reference, encrypted provider store and weekly uniqueness tests pass | Browser covers one-button weekly flow, write-only setup, fixed KPI cards, structured handover and same-week disablement | local fake Responses API only | no | no | Separately billed API key, provider data controls, quality and cost remain external |
...
| Upgrade rehearsal | Migration assertions | Synthetic v10 invoice/report data migrates losslessly through migration 19 | synthetic | n/a | no | No production or Vendure major-version upgrade performed |
...
- Backend: `cargo fmt --all -- --check`; SQLx-offline Clippy for all workspace targets with warnings
  denied; migrations 1–19 and `cargo sqlx prepare --workspace --check` against disposable
  PostgreSQL 16; 19 DB + 13 domain + 8 PDF + 55 server tests = **95 passed**.
- Frontend: production build and lint pass. Two Playwright Chromium flows cover the scoped
  ...
  fixture/manual report lifecycle, aggregate exports, weekly strategy/handover rendering, fixed KPI
  charts, ERP/mutation boundary probes and zero serious/critical axe violations.
...
- Upgrade: migration-10 synthetic invoice/report data reaches migration 18 losslessly and creates
  the weekly AI uniqueness boundary.
- Security: Amazon/OpenAI transport contracts, repository secret scan (258 files), dependency gate and
